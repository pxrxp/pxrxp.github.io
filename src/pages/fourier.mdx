...
import ExpX from '../components/ExpX.jsx'
import HelixCosSin from '../components/HelixCosSin.jsx'
import IotaCycle from '../components/IotaCycle.jsx'
import TangentCos from '../components/TangentCos.jsx'
import TangentSin from '../components/TangentSin.jsx'
import TaylorExpansionCos from '../components/TaylorExpansionCos.jsx'
import TaylorExpansionSin from '../components/TaylorExpansionSin.jsx'
import SlopeLine from '../components/SlopeLine.jsx'
import NumberLineMulNegOne from '../components/NumberLine.jsx'
import ReImRot from '../components/ImaginaryRotation.jsx'
import LinearApproximationTaylor from '../components/LinearApproximationTaylor.jsx'

# Journey from Euler to Fourier
...
- [Taylor Series Expansion of Common Functions](#taylor-series-expansion-of-common-functions)
  ...
  - [Finding Expressions for cos(x), sin(x)](#finding-expressions-for-cosx-sinx)
...
<br/>
For $f(x)=x^3$, the method would output
* $a_3 = 1$,
...
---
<br/>
The leap of faith we took to approximate $\cos x$ is actually called the **Maclaurin series**.
<br/>
...
<br/>
We actually just discovered **derivative**.
<br/>
---
...
Let's try to calculate the series for some functions:

$f_1(x) = \cos x=\text{ }?$

$f_2(x) = \sin x=\text{ }?$
<br/>
But we don't know the derivatives of $f_1(x)$ or $f_2(x)$

* We also **don't have a formula** for $\cos x$ or $\sin x$. That's actually what sparked the search for an approximation formula and study of Taylor series.

* **Binomial theorem isn't suitable** here.

* Actually, the whole $f'(x)=\lim_{\Delta x\to 0}\frac{\Delta f}{\Delta x}$**definition method isn't suitable** here.<br/><br/>
 $\because$ We **don't have sufficient knowledge** of **trigonometry and limits** yet.
...
for **specific functions** with a **well-defined graph** e.g. $\cos x$, $\sin x$.

we can try the **graphical method** to find the **expression for derivatives**.
<br/>
$f'(x)$ is just the **slope of the tangent line** to the $f(x)$.
...
<br/>
---
### Finding Expressions for cos(x), sin(x)
<br/>
...
$\therefore \cos x$ can be approximated at $x = a$ as:

$\cos x \approx 1 - \dfrac{(x-a)^2}{2!} + \dfrac{(x-a)^4}{4!} - \dfrac{(x-a)^6}{6!} + \dots + \dfrac{x^{^N}}{N!} \text{ }$ [N is even]

<br/>

$\therefore \sin x$ can be approximated at $x = a$ as:

$\sin x = x - \dfrac{(x-a)^3}{3!} + \dfrac{(x-a)^5}{5!} - \dfrac{(x-a)^7}{7!} + \dots + \dfrac{x^{^N}}{N!} \text{ }$ [N is odd]
...
Let's get back to our roots. The whole discussion started with complex numbers and the unit circle. Let's get back to it!
...
Just like $\cos x$ and $\sin x$, this is a new function we have discovered.

We can define it as:

$$
\begin{aligned}
\exp\;(i\theta) &= 1 + i\theta + \frac{(i\theta)^2}{2!} + \frac{(i\theta)^3}{3!} + \ldots\\
\\
...
### Euler's Identity
$$
\therefore\boldsymbol{e^{i\theta}} = \boldsymbol{\cos (\theta) + i \sin (\theta)}
$$
<br/>
$\Longrightarrow e^{i\pi}=-1$ (called the most beautiful equation in mathematics)

(Alternative way of writing $1\angle 180^\circ= -1$)
<br/>
---
<br/>
Let's play a game! Guess which function is this.
<details>
<summary>Reveal the answer.</summary>
It's actually $f(x) = x$.

Congrats, if you got it, because I wouldn't.

From this view, $x^3$, $2x$, $1753x$, $x^7$, $x^{153}$ $\ldots$ all look the same.
<br/>
This is because we are actually familiar with seeing $f(x) \text{ vs }x$ plots.

This one is $f(x)$ only.
We evaluated the function at many points and plotted them on a vertical number line.
</details>

<br/>
Guess what, we have been making the same mistake for $f(\theta)=e^{i\theta}$

The unit circle was just the $f(\theta)$. We need to actually add a third axis and plot $f(\theta) \text{ vs }\theta$.
